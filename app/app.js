...
// Sincronización híbrida (Local + Base de datos)
async function save() {
  localStorage.setItem('crm_leads', JSON.stringify(leads));
  localStorage.setItem('crm_nextId', String(nextId));
  
  // Guardado espejo en la nube si Supabase está configurado
  if (SUPABASE_URL !== "https://TU_PROYECTO.supabase.co") {
     try {
       await fetch(`${SUPABASE_URL}/rest/v1/leads_backup`, {
         method: 'POST',
         headers: {
           'apikey': SUPABASE_KEY,
     ...
     } catch (err) { console.error("Error sincronizando base de datos externa: ", err); }
  }
}
...
// RENDER REPARADO: No machaca los contenedores estructurales del HTML
function renderDashboard() {
  ...
  // Actualizar las métricas en los bloques estables de la pantalla
  if(document.getElementById('kpi-total')) document.getElementById('kpi-total').textContent = total;
  if(document.getElementById('kpi-ganados')) document.getElementById('kpi-ganados').textContent = ganados;
  if(document.getElementById('kpi-tasa')) document.getElementById('kpi-tasa').textContent = `${tasa}%`;
  if(document.getElementById('kpi-monto')) document.getElementById('kpi-monto').textContent = fmxn(monto);
  if(document.getElementById('kpi-potencial')) document.getElementById('kpi-potencial').textContent = fmxn(potentiel);
  if(document.getElementById('kpi-vencidos')) document.getElementById('kpi-vencidos').textContent = vencidos;
  if(document.getElementById('kpi-abandonados')) document.getElementById('kpi-abandonados').textContent = abandonados;
  if(document.getElementById('kpi-perdidos')) document.getElementById('kpi-perdidos').textContent = perdidos;

  // Renderizado dinámico de Gráficos de barras
  ...
  const chartFuente = document.getElementById('chart-fuente');
  if(chartFuente) {
    chartFuente.innerHTML = fSort.length ? fSort.map(([k, v], i) => `<div class="bar-row"><div class="bar-label" title="${k}">${k.length > 13 ? k.slice(0, 12) + '…' : k}</div><div class="bar-track"><div class="bar-fill" style="width:${Math.round(v / maxF * 100)}%;background:${BAR_COLORS[i % 10]}"></div></div><div class="bar-count">${v}</div></div>`).join('') : '<div class="empty">Sin registros</div>';
  }

  const pE = {}; ESTADOS.forEach(e => pE[e] = 0); leads.forEach(l => { pE[l.estadoLead] = (pE[l.estadoLead] || 0) + 1 });
  const chartPipeline = document.getElementById('chart-pipeline');
  if(chartPipeline) {
    chartPipeline.innerHTML = total > 0 ? ESTADOS.filter(e => pE[e]).map((e, i) => `<div class="bar-row"><div class="bar-label" style="font-size:11px" title="${e}">${e.length > 14 ? e.slice(0, 13) + '…' : e}</div><div class="bar-track"><div class="bar-fill" style="width:${Math.round(pE[e] / total * 100)}%;background:${ESTADO_COLORS[e]}"></div></div><div class="bar-count">${pE[e]}</div></div>`).join('') : '<div class="empty">Sin registros</div>';
  }
  ...
  const chartResponsable = document.getElementById('chart-responsable');
  if(chartResponsable) {
    chartResponsable.innerHTML = rSort.length ? rSort.map(([k, v], i) => `<div class="bar-row"><div class="bar-label" style="font-size:11px" title="${k}">${k.length > 14 ? k.slice(0, 13) + '…' : k}</div><div class="bar-track"><div class="bar-fill" style="width:${Math.round(v / maxR * 100)}%;background:${BAR_COLORS[(i + 2) % 10]}"></div></div><div class="bar-count">${v}</div></div>`).join('') : '<div class="empty">Sin registros</div>';
  }
  ...
  if(chartProductos) {
    chartProductos.innerHTML = prSort.length ? prSort.map(([k, v], i) => `<div class="bar-row"><div class="bar-label" style="font-size:11px" title="${k}">${k.length > 14 ? k.slice(0, 13) + '…' : k}</div><div class="bar-track"><div class="bar-fill" style="width:${Math.round(v / maxP * 100)}%;background:${BAR_COLORS[(i + 4) % 10]}"></div></div><div class="bar-count">${v}</div></div>`).join('') : '<div class="empty">Sin registros</div>';
  }
  ...
  const tableSeguimientos = document.getElementById('dashboard-seguimientos-body');
  if(tableSeguimientos) {
    tableSeguimientos.innerHTML = activos.length ? activos.map(l => `<tr onclick="openLead(${l.id})"><td style="font-size:12px;color:var(--text2)">#${l.id}</td><td><div class="td-name">${l.nombre}</div><div class="td-muted">${l.empresa}</div></td><td><span class="badge ${STATUS_CLASS[l.estadoLead] || ''}">${l.estadoLead}</span></td><td style="font-size:12px">${l.responsable}</td><td style="font-size:12px;font-weight:${l.proximoSeg < today() ? '600' : '400'};color:${l.proximoSeg < today() ? '#a32d2d' : 'var(--text)'}">${l.proximoSeg || '—'}</td><td style="font-size:12px;font-weight:500;color:var(--green)">${fmxn(l.montoPotencial)}</td></tr>`).join('') : '<tr><td colspan="6"><div class="empty">No hay seguimientos agendados</div></td></tr>';
  }
}
...
function renderConfig() {
  document.getElementById('tab-config').innerHTML = `
    <div class="config-container">
      ...
      <p style="color:var(--text2); margin-bottom:12px;">Modifica los elementos disponibles en los menús desplegables comerciales de tu CRM.</p>
...
function addConfigItem(type) {
  ...
  if (!val) return;
  if (!checkPasswordPrompt(`Agregar el elemento "${val}"`)) return;
...
function removeConfigItem(type, index) {
  if (!checkPasswordPrompt(`Eliminar este elemento`)) return;
